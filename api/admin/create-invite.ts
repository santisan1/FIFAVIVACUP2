...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { userId, tournamentId } = readBody<{ userId?: unknown; tournamentId?: unknown }>(req);
    if (typeof userId !== 'string' || typeof tournamentId !== 'string') return res.status(400).json({ error: 'userId y tournamentId requeridos' });
