...
import { methodNotAllowed, readBody, type VercelRequest, type VercelResponse } from '../_lib/http';

export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') return methodNotAllowed(res);
  try {
    const { token } = readBody<{ token?: unknown }>(req);
    ...
    if (!userDoc.exists) return res.status(404).json({ error: 'Jugador no encontrado' });
