...
export function DrawReveal({ participants }) {
  ...
  useEffect(() => {
    if (!started || names.length === 0 || currentPair >= pairs.length) return undefined;
  ...
  }, [started, names, currentPair, pairs.length]);
  ...
  return (
    <section className="glass rounded-3xl p-5 shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button className="btn btn-primary" onClick={() => { setStarted(true); setCurrentPair(0); }} disabled={pairs.length === 0}>
          <Sparkles className="h-4 w-4" /> Empezar show
        </button>
      </div>

      {started && currentPair < pairs.length && (
      ...
      {started && currentPair >= pairs.length && <p className="mt-4 rounded-2xl bg-winner/10 p-3 text-center text-sm font-black text-winner"><Trophy className="mr-2 inline h-4 w-4" /> ¡Sorteo completo! Ya están todos los cruces de octavos.</p>}
    </section>
  );
}
