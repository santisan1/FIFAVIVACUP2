...
export function AppShell({ children }) {
  ...
  const publicMode = pathname.startsWith('/tournament/') || pathname.startsWith('/screen/');
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-night/75 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <nav className="flex items-center gap-2 text-xs md:text-sm">
            {!playerMode && <NavLink to="/season/2026" className="btn btn-ghost px-3 py-2">Ranking</NavLink>}
            {publicMode && <NavLink to={pathname.replace('/tournament/', '/screen/')} className="btn btn-ghost px-3 py-2"><Monitor className="h-4 w-4" /> Pantalla</NavLink>}
