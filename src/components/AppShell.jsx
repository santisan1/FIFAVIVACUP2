...
export function AppShell({ children }) {
    return (<div className="min-h-screen overflow-hidden bg-night text-white">
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-night/75 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <Link to="/" className="flex items-center gap-3 font-black tracking-tight">
            ...
            <span>FIFA Viva Cup</span>
          </Link>
          <nav className="flex items-center gap-2 text-xs md:text-sm">
            <NavLink to="/season/2026" className="btn btn-ghost px-3 py-2">Ranking</NavLink>
            <NavLink to="/admin" className="btn btn-ghost px-3 py-2"><Shield className="h-4 w-4"/> Admin</NavLink>
          </nav>
        </div>
      </header>
