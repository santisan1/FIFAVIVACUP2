    ...
    <div className="glass overflow-hidden rounded-3xl shadow-card">
      <table className="w-full text-sm">
        <thead className="bg-white/5 text-left text-xs uppercase tracking-[.2em] text-slate-400">
          <tr><th className="p-4">Jugador</th><th className="p-4">Goles</th><th className="hidden p-4 sm:table-cell">Goleadores</th></tr>
        </thead>
        <tbody>
          {rows.map((row, index) => {
            ...
            return (
              <tr key={row.playerOwnerId} className={`border-t border-white/10 ${highlighted ? 'bg-electric/10' : ''}`}>
                ...
                <td className="hidden p-4 text-slate-300 sm:table-cell">{row.scorers.join(', ')}</td>
              </tr>
            );
          })}
          {rows.length === 0 && <tr><td className="p-4 text-sm text-slate-400" colSpan="3">Todavía no hay goles cargados.</td></tr>}
        </tbody>
      </table>
    </div>
  );
