  ...
  return <section className="glass rounded-3xl p-5 shadow-card"><h2 className="text-2xl font-black">Resultados rápidos</h2><p className="text-sm text-slate-400">Ahora podés cargar ida y vuelta en una misma card (4 casilleros) y ver el global antes de cerrar la serie.</p><div className="mt-4 grid gap-3 xl:grid-cols-2">{groupedMatches.map((group) => {
    ...
    return <div key={`${leg1.round}-${leg1.bracketPosition}-${leg1.id}`} className="rounded-3xl bg-white/5 p-4"><p className="text-xs font-black uppercase tracking-[.2em] text-electric">{roundLabels[leg1.round]}{group.type === 'twoLegs' ? ' · Ida y vuelta' : ''}</p><p className="mt-1 text-sm"><b>{leg1.playerAName || 'Jugador A'}</b> vs <b>{leg1.playerBName || 'Jugador B'}</b></p>
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        <label className="rounded-2xl bg-black/20 p-3 text-xs font-black uppercase tracking-[.2em] text-slate-300">Ida<div className="mt-2 grid grid-cols-2 gap-2"><input className="input text-center text-xl font-black" type="number" min="0" value={scoreDrafts[leg1.id]?.scoreA ?? ''} onChange={(e) => setDraft(leg1.id, 'scoreA', e.target.value)} /><input className="input text-center text-xl font-black" type="number" min="0" value={scoreDrafts[leg1.id]?.scoreB ?? ''} onChange={(e) => setDraft(leg1.id, 'scoreB', e.target.value)} /></div></label>
        <label className="rounded-2xl bg-black/20 p-3 text-xs font-black uppercase tracking-[.2em] text-slate-300">Vuelta<div className="mt-2 grid grid-cols-2 gap-2"><input className="input text-center text-xl font-black" type="number" min="0" disabled={!leg2} value={scoreDrafts[leg2?.id]?.scoreA ?? ''} onChange={(e) => leg2 && setDraft(leg2.id, 'scoreA', e.target.value)} /><input className="input text-center text-xl font-black" type="number" min="0" disabled={!leg2} value={scoreDrafts[leg2?.id]?.scoreB ?? ''} onChange={(e) => leg2 && setDraft(leg2.id, 'scoreB', e.target.value)} /></div></label>
      </div>
