...
export function TournamentPage() {
  ...
  useEffect(() => {
    ...
    setFinalStep(0);
    setShowAnnualTable(false);
    const timers = [setTimeout(() => setFinalStep(1), 1200), setTimeout(() => setFinalStep(2), 2600), setTimeout(() => setFinalStep(3), 4300)];
    return () => timers.forEach((timer) => clearTimeout(timer));
  }, [tournament?.status]);

  useEffect(() => {
    if (tournament?.status !== 'finished' || tournamentResults.length > 0) return undefined;
    const timer = setTimeout(async () => {
      if (!tournament) return;
      const refreshed = await listTournamentResultsBySeason(tournament.season);
      setTournamentResults(refreshed.filter((result) => result.tournamentId === id));
    }, 2000);
    return () => clearTimeout(timer);
  }, [id, tournament, tournament?.status, tournamentResults.length]);
        ...
        <section className="glass rounded-[2rem] p-6 shadow-card">
          ...
          {finalStep >= 3 && (
            <div className="mt-6">
              ...
              <p className="text-sm text-slate-300">Primero ves el torneo puntual. Después tocás siguiente y aparece la tabla anual actualizada.</p>
              ...
              <div className="mt-4">
                {!showAnnualTable ? (
                  <button type="button" className="btn btn-primary" onClick={() => setShowAnnualTable(true)}>Siguiente: mostrar tabla anual acá</button>
                ) : (
                  <Link className="btn btn-ghost" to={`/season/${tournament.season}`}>Ir al detalle completo del ranking anual</Link>
                )}
              </div>
              {showAnnualTable && (
                <div className="mt-5 rounded-3xl bg-white/5 p-4">
                  <h4 className="text-lg font-black">Tabla anual actualizada</h4>
                  <div className="mt-3 space-y-2">
                    {ranking.map((row, index) => (
                      <div key={row.playerId} className="flex items-center justify-between rounded-2xl bg-black/20 p-3 text-sm">
                        <span>#{index + 1} {row.nickname}</span>
                        <b className="text-electric">{row.points} pts</b>
                      </div>
                    ))}
                  </div>
                </div>
